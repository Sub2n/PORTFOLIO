...
import usePrevious from './usePrevValue.js';
...
const ReactPageScroller = ({
  ...
  const scrollWindowDown = useCallback(() => {
    if (!isScrolling && !blockScrollDown) {
      if (!isNil(containers[componentIndex + 1])) {
        disableScroll();
        isScrolling = true;
        console.log(pageContainer.current, componentIndex);
  ...
  useEffect(() => {
    ...
    const current = pageContainer.current;
    return () => {
      current.removeEventListener(Events.TOUCHMOVE, touchMove);
      current.removeEventListener(Events.KEYDOWN, keyPress);
    };
  }, [touchMove, keyPress]);
